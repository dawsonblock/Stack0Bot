...
Examples:
  ...
  if (trimmed === "runtime" || trimmed.startsWith("runtime ") || trimmed === "gateway" || trimmed.startsWith("gateway ")) {
    const { handleRuntime } = await import("../../commands-runtime.js");
    await handleRuntime(trimmed.replace(/^(runtime|gateway)\s*/, "").trim(), ctx);
    return true;
  }
