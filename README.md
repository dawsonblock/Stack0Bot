...
This repository is a bounded local agent stack. The supported execution path is:

`apps/shell` → `services/run-api` → `packages/agent-kernel` → `services/runtime-gateway` → `vendor/omlx-main`

The supported shell is the thin operator surface in `apps/shell`. `vendor/gsd-2-main` is retained as a source integration reference, not as the kernel or the default build target.
...
- Executable promotion validators that run bounded test and lint commands on staged post-patch worktrees.
